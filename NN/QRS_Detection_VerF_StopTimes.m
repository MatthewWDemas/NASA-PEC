        ...
        for i=1: length(Files)
                ...
                if (Protocol<='D')
                    ...
                    if Run==1 % TRAIN
                         %---------- Data Preprocess -------------------
                         Stamps=TrainS{S,j+1};
                         Temp=Data(ceil((Stamps(1)-Offset)*Fs):floor((Stamps(2)-Offset)*Fs),7);
                         %----------------------------------------------
                ...
                elseif (Protocol>'D')
                    ...
                    elseif Run==2;  % HYPOXIC 
                         ...
                         Temp=Data(ceil((Stamps(1)-Offset)*Fs):floor((Stamps(2)-Offset)*Fs),7);
                         %---------------------------------------------- 
                         [hrv, R_t, R_amp, R_index, S_t, S_amp]  = rpeakdetectNapoli(Temp,7,Fs); 
                         Hypoxic{S,j}=[[hrv,nan];R_t;R_amp;R_index];
...
save('HRV_SubMATB_SIM_Groups_Total_StarStopTime','Train','Hypoxic','NonHypoxic')