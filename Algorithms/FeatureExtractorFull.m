...
%                FeatureM(i,54)=Perform(i,12);
       ...
       %------------Demographics---------------------------------------------------
        ...
        FeatureM(i,85)=((Pt2-Pt1)/TL)/DempTemp(4) ; % Intensity from Average HR/MAx HR 
        FeatureM(i,86)=BMI*FeatureM(i,3); % BMI * HR Entropy
        FeatureM(i,87)=FeatureM(i,3)*DempTemp(1); % HR Entropy and AGE 
        FeatureM(i,88)=FeatureM(i,3)*DempTemp(2); % HR Entropy Total Flight Hours 
        FeatureM(i,89)=FeatureM(i,21)*((Pt2-Pt1)/TL)/DempTemp(4) ;  % HR Intensity and Complexity of Respiration Vol 
        FeatureM(i,90)=FeatureM(i,31)*DempTemp(5) ; % Coupling of RR and Respiration 
        FeatureM(i,91)=FeatureM(i,31)*DempTemp(6) ; % Coupling of RR and Respiration 
        FeatureM(i,92)=FeatureM(i,31)*DempTemp(5)*Demp(1) ; % Coupling of RR and Respiration * BMI * Age 
        FeatureM(i,93)=FeatureM(i,31)*DempTemp(5)*Demp(1)*Demp(2);  % Coupling of RR and Respiration * BMI * Age * Flight Hours 
        FeatureM(i,94)=FeatureM(i,31)*DempTemp(5)*Demp(1)*Demp(2)*(((Pt2-Pt1)/TL)/DempTemp(4) );  % Coupling of RR and Respiration * BMI * Age * Flight Hours* Intensity 
        FeatureM(i,95)=FeatureM(i,3)*DempTemp(5)*Demp(1)*Demp(2)*(((Pt2-Pt1)/TL)/DempTemp(4) );  % HR complexity * BMI * Age * Flight Hours* Intensity 
        FeatureM(i,96)=FeatureM(i,3)*DempTemp(5)*Demp(1)*Demp(2);  % Coupling of RR and Respiration * BMI * Age * Flight Hours 
        FeatureM(i,97)=FeatureM(i,3)*DempTemp(5)*Demp(1); 
        
        FeatureM(i,98)=O2Var*DempTemp(5)*Demp(1) ; % O2Var  * BMI * Age 
        FeatureM(i,99)=O2Var*DempTemp(5)*Demp(1)*Demp(2);  % O2Var  * BMI * Age * Flight Hours 
        FeatureM(i,100)=O2Var*DempTemp(5)*Demp(1)*Demp(2)*(((Pt2-Pt1)/TL)/DempTemp(4) );  % O2Var * BMI * Age * Flight Hours* Intensity 
        FeatureM(i,101)=O2Var*DempTemp(5)*Demp(1)*Demp(2)*(((Pt2-Pt1)/TL)/DempTemp(4) );  % O2Var * BMI * Age * Flight Hours* Intensity 
        FeatureM(i,102)=O2Var*DempTemp(5)*Demp(1)*Demp(2);  % O2Var * BMI * Age * Flight Hours 
        FeatureM(i,103)=FeatureM(i,36)*DempTemp(5)*Demp(1); 
        FeatureM(i,104)=FeatureM(i,36)*DempTemp(5)*Demp(1)*Demp(2)*(((Pt2-Pt1)/TL)/DempTemp(4) );  % O2Var  * BMI * Age * Flight Hours* Intensity 
        FeatureM(i,105)=FeatureM(i,36)*DempTemp(5)*Demp(1)*Demp(2)*(((Pt2-Pt1)/TL)/DempTemp(4) );  % O2Var * BMI * Age * Flight Hours* Intensity 
        FeatureM(i,106)= FeatureM(i,36)*DempTemp(5)*Demp(1)*Demp(2);  % O2Var  * BMI * Age * Flight Hours 
        FeatureM(i,107)= FeatureM(i,36)*DempTemp(5)*Demp(1);   % O2Var    * BMI * Age 
